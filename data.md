# Portfolio Data — Karuppasamy M

> Complete data export from the portfolio site. Source: `src/data/portfolioData.ts` and section components.

---

## Site Meta

| Field | Value |
|-------|-------|
| **Title** | Mr K \| Founder & CEO — Building AI Companies That Create Real Impact |
| **Description** | Founder portfolio of Mr K — Founder & CEO of Mr K AI Ecosystem, Mentor, Software Engineer, AI Builder, and Motivational Speaker building intelligent companies that create real impact. |
| **Keywords** | Founder, CEO, AI Builder, Mentor, Motivational Speaker, Mr K, AI Ecosystem, Startup Founder, Portfolio |
| **Brand** | Mr K AI Ecosystem |
| **Tech Stack (Site)** | React, TypeScript, Tailwind CSS, Vite, Framer Motion, Three.js |

---

## Personal Information

| Field | Value |
|-------|-------|
| **Name** | KARUPPASAMY M |
| **Display Name** | Mr K |
| **Role** | Founder & CEO of Mr K AI Ecosystem |
| **Email** | karuppasamy.murugesan02@gmail.com |
| **GitHub** | https://github.com/Mani-02nev |
| **LinkedIn** | https://www.linkedin.com/in/karuppasamy-m-b39995377/ |
| **Instagram** | https://www.instagram.com/mr_k.inc/ |
| **Calendly** | https://calendly.com/ |
| **Location** | Trichy, India |
| **Availability** | Open for MNC opportunities and high-impact software engineering roles |
| **Relocation** | Open to relocation, co-located programs, and global opportunities |

### Bio

As a Computer Engineering graduate, I've built a solid foundation in computer science principles. My journey is defined by a relentless drive to solve complex problems using intelligent systems, enterprise AI solutions at VDart, and developer community leadership through Google programs.

### Hero Tagline

- **Badge:** Founder & CEO
- **Greeting:** Hi, I'm Mr K
- **Headline:** I Build AI Companies That Create Real Impact.
- **Subtext:** Founder of Mr K AI Ecosystem — building intelligent systems, mentoring the next generation of builders, and speaking on the future of AI-driven entrepreneurship.
- **Hero Image:** /images/hero-image.png
- **CTA Primary:** View Work
- **CTA Secondary:** Let's Talk

### About Section Highlights

- **Tagline:** Engineering Maturity Meets Innovation
- **Academic Foundation:** Currently pursuing degree at Seshasayee Institute of Technology in Tiruchirappalli.
- **Professional Experience Summary:** Applied AI and systems engineering skills across enterprise and startup settings — from Novi Tech analytics dashboards to VDart's AI/ML automation pipelines and Google's campus developer community.
- **Approach:** Holistic — considers DevOps efficiency, user experience, and long-term scalability.

---

## Stats & Metrics

| Label | Value |
|-------|-------|
| Technical Depth | High |
| Learning Velocity | Rapid |
| Leadership | Core |
| Products | 5+ |

### ChatBot Performance Metrics

| Metric | Value |
|--------|-------|
| Learning Velocity (6 months) | +142% |
| Model Accuracy | 94.8% |
| Git Commits | 240+ |
| SaaS Deployments | 5 |
| Weekly Avg Commits | 60 commits/wk |
| Daily Avg Coding Hours | 7.3 hrs/day |

### Domain Proficiency (ChatBot)

| Domain | Proficiency |
|--------|-------------|
| AI & Machine Learning | 95% |
| Full-Stack React/Next.js | 90% |
| Data Engineering & SQL | 85% |
| Cloud & DevOps | 80% |

---

## Navigation

| Section | Anchor |
|---------|--------|
| Home | #home |
| Journey | #journey |
| Companies | #companies |
| Products | #products |
| Mentorship | #mentorship |
| Speaking | #speaking |
| Gallery | #gallery |
| Contact | #contact |

---

## Experience & Education

### 1. Computer Engineering Student

| Field | Value |
|-------|-------|
| **Company** | Seshasayee Institute of Technology |
| **URL** | https://www.sittrichy.in/ |
| **Location** | Tiruchirappalli |
| **Period** | 2025 - 2027 |
| **Type** | Education |

**Description:** Pursuing a comprehensive curriculum in Computer Engineering, with a focus on modern software systems and emerging technologies.

**Achievements:**
- Major in Computer Engineering with specialization in AI & Systems.
- Engaged in advanced technical studies and practical lab work.
- Building strong fundamentals in algorithms and system architecture.
- Active participant in hackathons, coding competitions, and tech clubs.

**Technologies:** Computer Science, Algorithms, Systems Design, Engineering

---

### 2. Data Analytics Intern

| Field | Value |
|-------|-------|
| **Company** | Novi Tech Company |
| **URL** | https://novitechrd.com/ |
| **Location** | Remote |
| **Period** | Dec 2023 – Jan 2024 |
| **Type** | Internship |
| **Certificate** | /certificates/KARUPPASAMY M (3).pdf |

**Description:** One-month intensive internship focused on data exploration, statistical analysis, and dashboard development.

**Achievements:**
- Performed comprehensive data cleaning and exploratory data analysis on diverse datasets.
- Designed and implemented interactive data dashboards for business intelligence.
- Applied statistical techniques to identify trends and patterns in complex numerical data.
- Generated detailed reports and presentations for technical and non-technical stakeholders.

**Technologies:** Data Analytics, Excel, Tableau, Statistics, SQL

---

### 3. AI & ML Engineering Intern

| Field | Value |
|-------|-------|
| **Company** | Novi Tech Company |
| **URL** | https://novitechrd.com/ |
| **Location** | Remote / Coimbatore |
| **Period** | Summer 2024 |
| **Type** | Internship |
| **Certificate** | /certificates/KARUPPASAMY M (2).pdf |

**Description:** Led development of intelligent systems and interactive user interfaces for AI-driven analytics tools at Novi Tech.

**Achievements:**
- Architected a real-time data visualization dashboard using React for AI performance monitoring.
- Implemented machine learning models for predictive analysis, integrating them into the frontend layer.
- Collaborated on designing high-end UX for complex data storytelling and AI interactions.
- Optimized system efficiency and frontend responsiveness for large-scale datasets.

**Technologies:** AI/ML, React, Python, Tailwind CSS, Chart.js

---

### 4. AI & ML Engineering Intern

| Field | Value |
|-------|-------|
| **Company** | VDart Inc. |
| **URL** | https://www.vdart.com/ |
| **Location** | Onsite / Tiruchirappalli |
| **Period** | 2025 |
| **Type** | Internship |
| **Certificate** | /certificates/vdart-internship.svg |

**Description:** Intensive AI & ML internship at VDart, a global staffing and technology solutions company, working on real-world machine learning and AI-driven automation projects.

**Achievements:**
- Developed and deployed machine learning models for enterprise automation workflows.
- Built AI pipelines integrating NLP and computer vision capabilities.
- Collaborated with senior engineers on data preprocessing and model optimization.
- Contributed to production-grade AI microservices and REST API integrations.

**Technologies:** Python, Machine Learning, NLP, Computer Vision, FastAPI, TensorFlow

---

### 5. Google Campus Ambassador

| Field | Value |
|-------|-------|
| **Company** | Google |
| **URL** | https://developers.google.com/ |
| **Location** | India |
| **Period** | 2026 - Present |
| **Type** | Ambassador |

**Description:** Selected as a Google Campus Ambassador for 2026, representing Google Developer ecosystem on campus, driving tech awareness and community engagement across the institution.

**Achievements:**
- Represent Google products, programs, and developer tools at the campus level.
- Organize and host developer events, workshops, and study jams for students.
- Drive awareness of Google Cloud, Android, and AI/ML platforms among peers.
- Bridge students with Google developer resources and career opportunities.

**Technologies:** Google Cloud, Android, AI/ML, Community Leadership, Developer Relations

---

## Skills

### Programming Languages
Java, Python, C, C++

### Core Technologies
HTML5, CSS3, JavaScript (ES6+), TypeScript

### Web Frameworks
React.js, Next.js, Django, Tailwind CSS

### App Frameworks
Flutter, React Native

### UI & Design Tools
Responsive Design, UI/UX Principles, Figma, Component Architecture

### AI & ML
TensorFlow, PyTorch, Scikit-learn, NLP, Computer Vision

### DevOps & Tools
Linux, Unix, DevOps, Git, GitHub

---

## Leadership & Soft Skills

| Skill | Description |
|-------|-------------|
| **Leadership** | Proven ability to guide teams towards shared goals, fostering a culture of excellence and accountability. |
| **Communication** | Expertise in translating complex technical concepts for non-technical stakeholders and cross-functional teams. |
| **People Management** | Committed to mentoring peers and managing group dynamics to optimize productivity and morale. |
| **Time & Task Management** | Detail-oriented approach to project timelines and task prioritization, ensuring high-quality delivery under pressure. |
| **Collaboration** | Prioritizing team success through active listening, conflict resolution, and shared responsibility. |
| **Professional Integrity** | Upholding high standards of ethics, reliability, and engineering discipline in every project. |
| **Adaptability** | Thrives in dynamic environments, quickly adjusting to new technologies and shifting project requirements. |
| **Empathy** | Understanding user needs and team perspectives to create inclusive solutions and foster a supportive work environment. |

---

## Products (Mr K AI Ecosystem)

### 1. Mr K AI Law Agent
- **Description:** Intelligent legal assistant powered by AI that provides case law analysis, contract review, and legal document insights with multi-domain legal expertise.
- **Live URL:** https://pickup-law-agent.vercel.app/
- **Tags:** AI Agent, Full Stack, React, NLP
- **Image:** /law_agent.png

### 2. AI HR Management System
- **Description:** AI-driven HR management platform with smart employee analytics, automated attendance tracking, leave management, and intelligent recruitment pipeline.
- **GitHub:** https://github.com/Mani-02nev/HRMS
- **Tags:** AI Platform, Full Stack, Python, Machine Learning
- **Image:** /hrms_dashboard.png

### 3. AI Sales Intelligence
- **Description:** Smart sales analytics platform leveraging AI for lead scoring, revenue forecasting, pipeline optimization, and actionable business insights.
- **GitHub:** https://github.com/Mani-02nev/Ai-Sales-Inteligence
- **Tags:** AI Platform, Full Stack, Python, Data Analytics
- **Image:** /sales_intelligence.png

### 4. AI Data Analytics + Assistant
- **Description:** Comprehensive data analytics platform with an integrated AI virtual assistant for natural language queries, real-time visualization, and automated insights.
- **GitHub:** https://github.com/Mani-02nev/Va_bot
- **Tags:** AI Agent, Data Analytics, Python, NLP
- **Image:** /data_analytics.png

### 5. AI Health Analytics & Sugar Predict
- **Description:** AI-powered health analytics tool that predicts blood sugar levels, assesses diabetes risk, and provides personalized dietary recommendations using ML models.
- **GitHub:** https://github.com/Mani-02nev/Suger-predect
- **Tags:** Machine Learning, Python, Health Tech, Data Analytics
- **Image:** /health_analytics.png

### 6. KS AI Resume Builder
- **Description:** AI-powered SaaS platform that generates ATS-optimized resumes instantly using structured prompt engineering.
- **Live URL:** https://kscv.vercel.app/
- **Tags:** AI SaaS, Full Stack, React
- **Image:** /ks_resume_builder.png

### 7. Agent K
- **Description:** Browser-based autonomous AI web agent built with scalable SaaS architecture, supporting multi-model orchestration.
- **Live URL:** https://agent02.vercel.app/
- **Tags:** AI Platform, Full Stack, React, Python
- **Image:** /agent_k_dashboard.png

### 8. UNIVAULT
- **Description:** Lightweight PHP-based e-commerce system using CSV flat-file storage instead of SQL databases for portability.
- **Live URL:** https://univalut.onrender.com/
- **Tags:** E-Commerce, PHP, Full Stack
- **Image:** /univault_ecommerce.png

### 9. AC Duct ERP System
- **Description:** Full-stack unified digital system for AC duct fabrication companies in Dubai, replacing manual workflows from inquiry to reporting.
- **Tags:** React, Node.js, PostgreSQL, React Native, Full Stack
- **Image:** /assets/ac-project-img/image-1.png

---

## Certifications

| # | Title | Issuer | Date | Certificate Link |
|---|-------|--------|------|------------------|
| 1 | Google Core Python Certification | Google | 2025 | /certificates/gogle.jpeg |
| 2 | VDart AI & ML Internship | VDart Inc. | 2025 | /certificates/vdart-internship.svg |
| 3 | Machine Learning Internship | Professional Development | 2025 | /certificates/KARUPPASAMY M (1).pdf |
| 4 | Artificial Intelligence Internship | Technical Institute | 2025 | /certificates/KARUPPASAMY M (2).pdf |
| 5 | Data Analytics Internship | Novi Tech Certification | 2025 | /certificates/KARUPPASAMY M (3).pdf |
| 6 | Artificial Intelligence 30day masterclass | Technical Excellence | 2025 | /certificates/KARUPPASAMY M.pdf |
| 7 | Artificial Intelligence ROAD MAP | Technical Excellence | 2025 | /certificates/HCL GUVI Certification - 1h6nM1sg71C5972067.png |
| 8 | 5-Day AI Agents Intensive Course | Google Cloud / technical Excellence | 2025 | /certificates/5-Day AI Agents Intensive Course with Google.png |
| 9 | MAANG Devloper | Career Excellence Track | 2025 | /certificates/maang.png |

---

## Contact

| Channel | Value |
|---------|-------|
| **Email** | karuppasamy.murugesan02@gmail.com |
| **LinkedIn** | https://www.linkedin.com/in/karuppasamy-m-b39995377/ |
| **GitHub** | https://github.com/Mani-02nev |

**Contact Form Fields:** Full Name, Email Address, Subject, Message

**CTA:** Let's Build Something Great Together

---

## Founder Stats

| Label | Value |
|-------|-------|
| Companies Built | 1+ |
| Products | 9+ |
| Developers Mentored | 50+ |
| Students Inspired | 200+ |
| Years Building | 3+ |

---

## About

### Mission

To build AI-powered companies and ecosystems that solve real-world problems with elegance, scalability, and measurable impact.

### Vision

A world where intelligent systems empower every founder, student, and engineer to build products that matter — led by the Mr K AI Ecosystem.

### Values

| Title | Description |
|-------|-------------|
| **Impact First** | Every product must create tangible value for real people and businesses. |
| **Excellence** | Engineering maturity meets innovation — no shortcuts, no compromise on quality. |
| **Leadership** | Guide teams and communities with empathy, clarity, and accountability. |
| **Continuous Learning** | Rapid learning velocity — stay ahead of the curve in AI and systems engineering. |
| **Integrity** | Professional ethics, reliability, and engineering discipline in every decision. |

### Philosophy

Build with purpose. Ship with precision. Lead with vision. Technology is a tool — impact is the outcome. I believe the best founders combine deep technical craft with the ability to inspire others to build alongside them.

---

## Companies

### 1. Mr K AI Ecosystem

| Field | Value |
|-------|-------|
| **Status** | Active |
| **Description** | A founder-led AI ecosystem building intelligent SaaS products across legal, HR, sales, health, and autonomous agent domains. |
| **Vision** | To become the premier AI product studio from India, shipping world-class intelligent systems that compete globally. |
| **URL** | https://agent02.vercel.app/ |

### 2. Future Ventures

| Field | Value |
|-------|-------|
| **Status** | Vision |
| **Description** | Expanding into AI infrastructure, developer tools, and education platforms that scale the Mr K mission globally. |
| **Vision** | Building the next generation of AI-native companies that redefine how businesses operate. |

---

## Mentorship

### Intro

Passionate about mentoring developers, students, and aspiring founders through hands-on guidance, community leadership, and real-world project experience.

### Stats

| Label | Value |
|-------|-------|
| Students Mentored | 50+ |
| Mentorship Sessions | 100+ |
| Communities Led | 3+ |
| Events Organized | 15+ |

### Communities

- Google Developer Student Community — Campus Ambassador Program
- Mr K AI Builder Community — Peer mentoring and project reviews
- Campus Tech Clubs — Hackathons and coding workshops

### Events

- Google Developer Study Jams — AI/ML and Cloud workshops
- Campus Hackathons — Technical mentorship and judging
- AI Product Demo Days — Showcasing Mr K ecosystem products

### Testimonials

> "Mr K's mentorship transformed how I approach building AI products. His clarity and technical depth are unmatched."
> — **Arjun S.** — Aspiring AI Engineer

> "Working under his guidance at community events gave me the confidence to ship my first full-stack project."
> — **Priya R.** — Computer Science Student

> "A true founder mindset — he doesn't just teach code, he teaches how to think like a builder."
> — **Rahul M.** — Junior Developer

---

## Speaking

### Intro

Motivational speaker and tech evangelist sharing insights on AI entrepreneurship, product building, and the future of intelligent systems.

### Philosophy

Speak to inspire action. Every talk should leave the audience with one clear idea they can implement immediately — whether that's shipping a product, joining a community, or rethinking their career path.

### Topics

- Building AI Companies from Zero to One
- The Future of Autonomous AI Agents
- From Student to Founder — A Builder's Journey
- Google Developer Ecosystem & Community Leadership
- Engineering Excellence in the Age of AI

### Talks

#### 1. Building the Mr K AI Ecosystem

| Field | Value |
|-------|-------|
| **Venue** | Seshasayee Institute of Technology |
| **Date** | 2025 |
| **Description** | Keynote on founding an AI product ecosystem while still in college — lessons on shipping, scaling, and staying focused. |

#### 2. AI Agents — The Next Frontier

| Field | Value |
|-------|-------|
| **Venue** | Google Developer Event |
| **Date** | 2026 |
| **Description** | Deep dive into autonomous AI agents, multi-model orchestration, and the Agent K platform architecture. |

### Events

- Google Campus Ambassador Launch — 2026
- AI Innovation Summit — SIT Trichy
- Developer Meetup — Trichy Tech Community

### Video Links

| Label | Value |
|-------|-------|
| Agent K Demo | https://agent02.vercel.app/ |
| Mr K AI Law Agent | https://pickup-law-agent.vercel.app/ |

---

## Impact

| Label | Value |
|-------|-------|
| People Reached | 500+ |
| Products Shipped | 9+ |
| Hours Mentored | 200+ |
| Events | 15+ |
| Countries | 2+ |

---

## Gallery

### 1. Founder Portrait

| Field | Value |
|-------|-------|
| **Category** | Portrait |
| **Image** | /images/hero-image.png |

### 2. Agent K Platform

| Field | Value |
|-------|-------|
| **Category** | Product |
| **Image** | /agent_k_dashboard.png |

### 3. AI Law Agent

| Field | Value |
|-------|-------|
| **Category** | Product |
| **Image** | /law_agent.png |

### 4. HR Management System

| Field | Value |
|-------|-------|
| **Category** | Product |
| **Image** | /hrms_dashboard.png |

### 5. Google Certification

| Field | Value |
|-------|-------|
| **Category** | Achievement |
| **Image** | /certificates/gogle.jpeg |

### 6. VDart Internship

| Field | Value |
|-------|-------|
| **Category** | Achievement |
| **Image** | /certificates/vdart-internship.svg |

---

## Testimonials

> "Mr K represents the new wave of AI founders — technically brilliant, community-driven, and relentlessly focused on impact."
> — **Tech Community Leader** — Google Developer Programs

> "His products aren't demos — they're production-grade systems that solve real business problems."
> — **Industry Mentor** — VDart Inc.

> "Rare combination of deep AI expertise and the ability to inspire an entire campus to start building."
> — **Faculty Advisor** — Seshasayee Institute of Technology

> "The Mr K AI Ecosystem is proof that student founders can compete with anyone globally."
> — **Peer Founder** — Startup Community

---

## Footer

| Field | Value |
|-------|-------|
| **Copyright** | © 2026 Mr K. All rights reserved. |
| **Tagline** | Made with passion. |
| **Privacy Policy** | # |
| **Terms of Service** | # |

---

## ChatBot — Mr K Analytics Engine

**Name:** Mr K Analytics Engine  
**Greeting:** Hi! I'm the Mr K Analytics Engine. How can I help you explore Karuppasamy's career metrics today?

### Suggested Queries
- Show last 6 months performance
- What is your technology stack?
- Show verified certifications
- How to contact Karuppasamy?

### Value Proposition
Karuppasamy brings rare technical versatility: combining solid systems foundations (Diploma in Computer Engineering) with practical AI/ML model deployment (VDart, Novi Tech) and community representation (Google Campus Ambassador). He works with high accuracy, fast turnaround, and values premium UX/UI styling.

## Assets Reference

### Company Logos
- Google, VDart Inc., Novi Tech Company, Seshasayee Institute of Technology (SIT)

### Profile Image
- /images/hero-image.png

### Product Images
- /law_agent.png
- /hrms_dashboard.png
- /sales_intelligence.png
- /data_analytics.png
- /health_analytics.png
- /ks_resume_builder.png
- /agent_k_dashboard.png
- /univault_ecommerce.png
- /assets/ac-project-img/image-1.png

### Certificate Files
- /certificates/gogle.jpeg
- /certificates/vdart-internship.svg
- /certificates/KARUPPASAMY M (1).pdf
- /certificates/KARUPPASAMY M (2).pdf
- /certificates/KARUPPASAMY M (3).pdf
- /certificates/KARUPPASAMY M.pdf
- /certificates/HCL GUVI Certification - 1h6nM1sg71C5972067.png
- /certificates/5-Day AI Agents Intensive Course with Google.png
- /certificates/maang.png
